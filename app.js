...
(function () {
  ...
  function priceHtml(loc) {
    ...
    if (!loc.price) return '';
    if (typeof loc.price === 'string') {                       // free-form tiered price -> compact headline + euro hint
      const head = loc.price.split(/[;(]/)[0].trim();
      const m = head.match(/([₩¥])\s*([\d,]+(?:\.\d+)?)/);
      let eur = '';
      if (m) { const cur = m[1] === '₩' ? 'KRW' : 'CNY'; const amt = parseFloat(m[2].replace(/,/g, '')); const e = amt * (rates[cur] || 0); if (e > 0) eur = ` <span class="eur">· ${'€' + (e >= 10 ? Math.round(e) : e.toFixed(1))}</span>`; }
      return `<span class="price">${esc(head)}${eur}</span>`;
    }
